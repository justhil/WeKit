...
| `.github/workflows/jev-ci.yml` | 推送 `jev` 分支时在 GitHub 上运行单测、`./x` 构建、APK 校验并上传可下载的调试版 APK；不修改上游 CI，不需要签名密钥 |
...
远程推送 `jev` 分支会触发 `.github/workflows/jev-ci.yml`：安装所需 Android SDK/NDK，运行 Jev 单测，再由 `./x` 编译 native 与标准版调试 APK，验证双格式安装包并上传 `wekit-jev-standard-debug` 工件。调试版使用构建机自己的签名，与本机安装的 APK 签名可能不同；不能直接用它覆盖安装已有版本。首次运行仍须在 GitHub 仓库启用 Actions。
