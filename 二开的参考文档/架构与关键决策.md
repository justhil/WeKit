...
> 适用于 `D:\workspace\wa-jev`（WeKit 二开，`jev` 分支）。最后更新：2026-09-24。
...
| 项 | 状态 |
|---|---|
| 分支 | `jev`，基于上游 `bdc7f18`；包含第一阶段与本轮方向语境、入库刷新改动 |
| 已实现 | 多维度分析：对方的消息分析阴阳怪气、调侃、网络黑话、敷衍、情绪；自己的消息分析回应、温度、语气，外加两项风险提示<br>结构化语境包：按对话段落截取、非文字占位符、引用回复、回复间隔、按发送方向筛选目标之后消息、每个聊天的背景、全局补充说明；新消息入库后可按位置范围刷新旧评价<br>界面：一行标签、详情面板、在面板里设置聊天背景、单条重新分析 |
| 未实现 | 大模型深度解读、关系记忆层、发送前提醒、会话报告、结果持久化（见 §12） |
| 最新构建 | 本地 APK SHA-256 `9068de4d0df7f68a7ffbcb920026d67ecf4435a0d81d8a8ebd8b5a1b25461227`：Jev 单测 23/23 通过，完整 `./x build --flavor standard` 通过；已安装到手机并重启微信，但本轮功能表现尚未真机验收 |
| 真机验证 | 用户确认上一版标签挂在正确气泡下，面板和分析正常；**本轮方向语境与入库刷新尚未经真机验证** |
...
| `.github/workflows/jev-ci.yml` | 推送 `jev` 分支时在 GitHub 上运行单测、`./x` 发布模式构建、APK 校验并上传可下载的 Beta APK；不修改上游 CI，不需要签名密钥 |
...
远程推送 `jev` 分支会触发 `.github/workflows/jev-ci.yml`：安装所需 Android SDK/NDK，运行 Jev 单测，再由 `./x build --release --flavor standard` 编译 native 与标准版发布模式 APK，验证双格式安装包并上传 `wekit-jev-standard-release-beta` 工件。Beta 仅指 CI 工件名称，不创建 GitHub Release 或标签；未配置发布密钥时仍使用构建机的调试签名，与本机安装的 APK 签名可能不同，不能直接覆盖安装已有版本。
...
| `WeDatabaseApi`：`isReady`、`getMessages`、`executeQuery`；`WeDatabaseListenerApi.IInsertListener` | 查询聊天记录；监听新消息入库以触发合批刷新 |
...
JevAnalysis（运行时，依赖微信和 WeKit 运行时）
  ├─ 监听消息视图的创建和挂载；监听消息入库后合批刷新最近位置范围内的评价
  ├─ 读数据库（所有消息类型，目标前后）→ 用 QuoteMessage 解析引用（native XML 解析器，桌面 JVM 上用不了）
  ├─ 请求队列、并发控制、版本失效、内存缓存
  └─ 气泡标签、详情面板、设置对话框
JevWindow（纯逻辑，可单测）
  └─ 数据库行 → 语境包：按段落截取、占位符、引用、截断、间隔、方向敏感的之后消息、匿名发言人；按聊天位置选择刷新目标
JevProtocol（纯逻辑，可单测）
...
```mermaid
flowchart TD
  A[视图绑定或消息入库] --> B{进入聊天 / 滚动 / 新消息?}
  B -- 进入聊天 --> E[查询最新 N 条及其前文]
  B -- 滚动 --> F[120ms 合批：可见目标前文与最新尾部]
  B -- 新消息 --> R[120ms 合批：最近配置的位置范围内的旧目标和新消息]
  E --> G[解析引用 QuoteMessage]
  F --> G
  R --> G
  G --> H[JevWindow 按发送方向组织前后文]
  H --> I{实际相关后文 ID 与缓存不同?}
...
| 模式 | 何时规划 | 读多少数据 | 说明 |
|---|---|---|---|
| 进入聊天分析最新 N 条（默认） | 进入会话时规划一次；新消息入库后另走增量刷新 | 最新 `max(N, 自己刷新范围, 对方刷新范围) + 上下文上限` 条 | 初次只为最新 N 条生成新请求；范围内更早的消息只有已缓存结果才会重新评价。历史滚动只显示缓存；入库刷新不需重新进入 |
| 跟随渲染 | 消息视图出现在屏幕上时；新消息入库后另走增量刷新 | 可见目标：最新尾部和目标之前 `上限 + 本批数量` 条；刷新时同上 | 120ms 合批；可见候选最多 32 个，队列最多 50 个；滚动分析的请求发出前已离屏则跳过，入库触发的刷新不依赖是否在屏幕上 |

### 4.6 缓存、失效与重新分析

- **缓存**：结果按消息 ID 存在内存 `LinkedHashMap` 里，最多 2000 条，重启微信后丢失。
- **重新分析**：新消息入库后，在最近配置的位置范围内选择旧目标（双方消息及非文字都占位置）；自己的新消息也可能改变自己的旧评价，对方新消息也可能改变自己旧消息的评价。缓存记录实际传入 `after` 的消息 ID 列表，列表变化才重发，不按回复数量是否增加判断。请求仍在进行时若又来了消息，完成后重新查询并比较一次。入库回调 120ms 合批；连续入库时新消息本身均纳入待分析范围。
- **失效**：
  - 更换密钥、修改目标之前条数、自己/对方刷新范围或补充说明：清空全部结果。
  - 调整最新消息条数或模式：重置活动聊天窗口，下次进入聊天后按新配置分析。
...
## 4.8 方向敏感的语境与增量刷新（已实现，待真机验收）

- 对方和自己的消息分别评价。`conversation` 只取目标之前的聊天记录，双方消息与非文字消息合计占位置，保持原时间顺序与匿名发言人；目标消息不重复放入前后文。
- `after` 只取目标之后已存在的消息。对方消息只看同一发送者后续发的消息（群聊按实际成员 ID 区分）。自己的消息看自己的后续消息及对方反应；单聊纳入对方后续，群聊仅纳入紧跟自己消息的回复，或明确引用当前目标文字的回复。群聊的相邻回复是保守启发式，不保证识别真实意图。目标在对应方向的最近位置范围内时，该范围内目标之后的消息全部参与筛选，不再单独设置事后消息条数；较早的可见消息不硬塞最新话题。进入聊天首次分析最新 N 条时，先用 N 扩大可用范围。
- 新消息**在当前打开的聊天中**入库时合批规划一次，不依赖气泡滚动或重绑；新消息本身也分析。刷新范围分别为自己和对方（默认各 20，0 禁止旧消息刷新），均按聊天列表中最新 N 个位置计数，再选出相应方向的旧文本评价；收到消息还可能改变自己的旧评价。队列上限 50，请求数受并发设置控制；增大刷新范围也会扩大事后参考窗口。切回聊天时会重新核对窗口内已有的缓存结果。
- **验收边界**：单聊收发、群聊不同成员、引用与非文字、刷新范围 0（不刷新旧评价）、快速连续到达、重绑去重、聊天切换；构建成功不代表真机刷新已验证。
...
| `after` | 目标在对应方向的最近刷新位置范围内时，取该范围里目标之后的**全部**消息，再筛选相关消息，按时间正序；对方消息只收同一发送者后续消息，自己的消息收自己后续消息和对方反应。单聊对方后续都纳入；群聊仅纳入紧随自己消息或引用当前目标文字的回复。进入聊天首次分析最新 N 条时，窗口至少覆盖这 N 条；可见目标若比刷新范围还早，不附带遥远的新话题。最近 5 条相关消息每条最多 300 字，更早的最多 40 字，避免大量历史文字导致请求超限 |
...
| 详情面板 | 对方的消息：阴阳怪气、调侃、情绪前三、敷衍、网络黑话的概率条<br>自己的消息：总评、三项得分、两项风险<br>底部：模型版本、token 数、实际附带几条目标之后的相关消息<br>另有"这个聊天的背景"输入框（≤ 100 字，点确定后保存并重新分析）和"重新分析"按钮 |
...
| `jev_context_count` | 30 | 目标之前的消息条数上限（0–200），双方与非文字都占位置，按对话段落截取；界面明确标注为目标之前 |
| `jev_self_refresh_count` | 20 | 新消息到达时，最近多少条消息位置内自己的旧评价可刷新（0–200） |
| `jev_other_refresh_count` | 20 | 新消息到达时，最近多少条消息位置内对方的旧评价可刷新（0–200） |
...
配置用 `WePrefs`（MMKV）保存，存储位置是宿主的私有目录 `filesDir/mmkv`。旧版 `jev_after_count` 设置已不再读取，也不显示在界面；分析结果只保存在内存里。

## 9. 隐私与安全边界

- **发送给 TypeSafe 的内容**：目标消息；目标之前上限以内的双方消息（文字按长度截断，其余只发占位符）；被引用的话；刷新范围内目标之后筛出的相关消息（较早的消息文字按长度截断）；聊天背景和补充说明。不会发送 wxid、昵称或备注。刷新范围与并发设置影响请求量和费用。
...
- **本轮合成样例复测**：旧报告 `jev-eval/results/20260924-154301-context/report.md` 的 `session+all` 为 0.95（7/8 对），当时评测脚本仍只取最后 3 条 `after`；修正脚本为保留全部样例后续消息后，新报告 `jev-eval/results/20260924-154759-context/report.md` 为 0.89（6/8 对）。两次均为 19 条合成样例、29 个唯一请求；不能判断差异来自后续消息变多还是模型波动，也没有覆盖最长 200 条的后续窗口。真实聊天准确率仍未知。
- **本轮新 APK 尚未安装和真机验证**：手机当前仍是上一版。入库事件能否覆盖所有消息路径、快速连续消息去重、群聊同一发送者筛选及相邻回复误收、切换聊天时刷新，均待验证。用户已确认更早版本的气泡挂载、详情面板与分析功能正常。
- **没有重试**：遇到 429/529 不做退避，失败了只能等下次触发再分析。
- **群聊回应识别的局限**：自己的消息之后，紧跟着出现的群成员发言只是可能在回应我；无引用时无法可靠证明归属，可能把相邻的无关发言纳入 `after`。引用当前目标文字的回复证据更强；这项行为必须用真机和真实匿名样例验证，不能宣称已准确识别所有群聊回应。
...
- 本项目评测的原始结果（本机，不进 git）：`jev-eval/results/20260923-195103/`（第一轮）、`20260923-201305/`（第二轮）、`20260923-211940-context/`（第三轮）、`20260924-131045-context/`（上一轮复测）、`20260924-154301-context/`（本轮复测）
